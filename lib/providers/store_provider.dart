...
class StoreProvider with ChangeNotifier{
  ...
  Future<void> getUserLocationData(context) async {
    _userServices.getUserById(user!.uid).then((result) {
      if (user != null) {
        this.userLatitude = (result.data() as Map)['latitude'];
        this.userLongitude = (result.data() as Map)['longitude'];
        notifyListeners();
      } else {
  ...
  Future<Position> determinePosition() async {
    ...
    if (!serviceEnabled) {
      return Future.error('Location services have been turned off.');
    }

    permission = await Geolocator.checkPermission();
    if (permission == LocationPermission.deniedForever) {
      return Future.error(
          'Location update permission denied, we cannot grant request permission.');
    }
    ...
    if (permission == LocationPermission.denied) {
      ...
      if (permission != LocationPermission.whileInUse &&
        ...
        return Future.error(
            'Location update permission denied (actual value: $permission).');
      }
    }
