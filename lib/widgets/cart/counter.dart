      ...
      if (querySnapshot.docs.isNotEmpty){
          querySnapshot.docs.forEach((doc) {
            if (doc['productId'] == widget.document.data()['productId']) {
              //Khi sản phẩm đã chọn đã tồn tại trong giỏ hàng,không cần thêm lại vào giỏ hàng
              if(mounted){
                setState(() {
                  _qty = doc['qty'];
                  _docId = doc.id;
    ...
    showCupertinoDialog(
        ...
        builder: (BuildContext context) {
          return CupertinoAlertDialog(
            title: Text('Thay thế các mặt hàng trong giỏ'),
            content: Text(
                'Giỏ hàng của bạn chứa các mặt hàng từ $shopName. Bạn có muốn hủy lựa chọn và thêm các mặt hàng từ  ${widget.document.data()['seller']['shopName']}'),
            actions: [
              FlatButton(
                child: Text(
                  'Không',
